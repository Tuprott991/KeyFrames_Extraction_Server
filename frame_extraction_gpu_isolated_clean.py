...
def extract_frames(video_path, skip_start=0, skip_end=0, resize_shape=RESIZE_SHAPE):
    ...
    # Try with software decoding for AV1 videos
    cap = cv2.VideoCapture(video_path)
    
    # If opening fails, try with explicit software decoding
    if not cap.isOpened():
        print(f"⚠️  Standard video open failed, trying with software decoding...")
        # Set environment variables for software decoding
        old_hwaccel = os.environ.get('OPENCV_FFMPEG_CAPTURE_OPTIONS')
        ...
        try:
            ...
            # Restore environment
            if old_hwaccel is None:
                os.environ.pop('OPENCV_FFMPEG_CAPTURE_OPTIONS', None)
            else:
                os.environ['OPENCV_FFMPEG_CAPTURE_OPTIONS'] = old_hwaccel
...
def extract_keyframes(video_path, scene_changes, frames, fps, start_frame, end_frame):
    ...
    # If opening fails, try with explicit software decoding
    if not cap.isOpened():
        print(f"⚠️  Standard video open failed for keyframes, trying with software decoding...")
        old_hwaccel = os.environ.get('OPENCV_FFMPEG_CAPTURE_OPTIONS')
        os.environ['OPENCV_FFMPEG_CAPTURE_OPTIONS'] = 'hwaccel;none'
    ...
    if not cap.isOpened():
        print(f"❌ Could not open video for keyframes: {video_path}")
        return {}, []
